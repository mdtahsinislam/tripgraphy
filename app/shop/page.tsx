...
export default function Shop() {
  ...
  const handleCheckout = async (item: ProductType) => {
    if (!session) {
      router.push("/login");
      return;
    }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <section className="relative min-h-[70vh] md:min-h-[85vh] flex items-center justify-center overflow-hidden">
        <div className="absolute inset-0">
          <div
            className="w-full h-full bg-cover bg-center bg-no-repeat"
            style={{ backgroundImage: `url('/images/shop-main.avif')` }}
          >
            <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/40 to-black/20" />
          </div>
        </div>
              ...
              {products.map((item, index) => (
                ...
                >
                  ...
                  <div className="p-5 sm:p-6">
                    ...
                    <button
                      onClick={() => handleCheckout(item)}
                      className="w-full py-3 px-6 bg-gradient-to-r from-amber-600 to-amber-500 hover:from-amber-700 hover:to-amber-600 text-white font-semibold rounded-full shadow-lg hover:shadow-xl transition-all duration-300 flex items-center justify-center gap-2"
                    >
                      Purchase
                    </button>
                  </div>
