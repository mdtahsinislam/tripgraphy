...
// //    { name: "Be A Tour Gide", href: "/tourgide" },
// //   ];
...
const Navbar = () => {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex justify-between items-center">
        ...
        <div className="hidden md:flex items-center gap-8 lg:gap-10 text-sm font-bold tracking-wider uppercase text-gray-700">
          ...
          <div
            ...
            onMouseLeave={() => setPagesOpen(false)}
          >
            <button className="hover:text-cyan-600 transition-colors flex items-center gap-1 uppercase">
              Pages
            </button>
            {pagesOpen && (
              <div className="absolute left-0 top-full w-64 bg-white shadow-xl border border-gray-100 rounded-lg py-3 z-50">
                {pageLinks.map((link) => (
                  <Link
                    key={link.name}
                    href={link.href}
                ...
                {shopLinks.map((link) => (
                  ...
                  >
                    {link.name}
                      ...
                      {/* Desktop Auth Section with NEW Dropdown */}
  ...
  {isAuthenticated ? (
    <div className="relative">
      ...
      >
        {userImage ? (
          <Image
            ...
            width={40}
            height={40}
            className="rounded-full object-cover border-2 border-cyan-500 shadow-sm"
          />
        ) : (
          <div className="w-10 h-10 rounded-full bg-cyan-100 flex items-center justify-center text-cyan-700 font-semibold border-2 border-cyan-500">
            {userName?.[0]?.toUpperCase() || "?"}
          </div>
        )}
      </button>
      ...
      {/* User Dropdown Menu */}
      {userMenuOpen && (
        <div className="absolute right-0 top-full mt-2 w-48 bg-white shadow-2xl border border-gray-100 rounded-xl py-2 z-[60]">
          <div className="px-4 py-2 border-b border-gray-50 mb-1">
            <p className="text-xs text-gray-400 font-semibold uppercase tracking-wider">Hello,</p>
            <p className="text-sm font-bold text-gray-800 truncate">{userName}</p>
          </div>
          <Link
            href="/dashboard"
            className="block px-4 py-2.5 text-gray-700 hover:text-cyan-600 hover:bg-cyan-50 transition-colors font-medium"
          >
            Dashboard
          </Link>
          <button
            onClick={() => signOut({ callbackUrl: "/" })}
            className="w-full text-left px-4 py-2.5 text-red-500 hover:bg-red-50 transition-colors font-medium border-t border-gray-50 mt-1"
          >
            Logout
          </button>
        </div>
    ...
    >
      LOGIN
        ...
        >
          <svg className="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d={open ? "M6 18L18 6M6 6l12 12" : "M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5"} />
          </svg>
        </button>
      </div>
      ...
      {/* Mobile Menu */}
      {open && (
        <div className="md:hidden bg-white border-t px-5 py-6 flex flex-col gap-6 text-base font-medium text-gray-800 h-screen overflow-y-auto">
          {/* User Profile Info for Mobile */}
          {isAuthenticated && (
            <div className="flex items-center gap-4 bg-gray-50 p-4 rounded-xl mb-2">
               {userImage ? (
                <Image src={userImage} alt="User" width={50} height={50} className="rounded-full" />
              ) : (
                <div className="w-12 h-12 rounded-full bg-cyan-100 flex items-center justify-center text-cyan-700 font-bold">{userName?.[0]}</div>
              )}
              <div>
                <p className="font-bold text-gray-900">{userName}</p>
                <Link href="/dashboard" onClick={() => setOpen(false)} className="text-sm text-cyan-600 font-semibold">View Dashboard</Link>
              </div>
            </div>
          )}

          <Link href="/" onClick={() => setOpen(false)} className="hover:text-cyan-600 font-bold border-b pb-2">Home</Link>

          {[
            { title: "PAGES", links: pageLinks },
            { title: "TRAVELS", links: travelsLinks },
            { title: "TRAVEL DIARIES", links: blogLinks },
            { title: "SHOP", links: shopLinks },
          ].map((section) => (
            <div key={section.title} className="flex flex-col gap-2.5">
              <p className="text-gray-400 text-xs font-bold tracking-widest">{section.title}</p>
              {section.links.map((link) => (
                <Link key={link.name} href={link.href} onClick={() => setOpen(false)} className="pl-2 hover:text-cyan-600">
                  {link.name}
                </Link>
              ))}
            </div>
          ...
          <div className="mt-auto pb-10">
            {isAuthenticated ? (
              <button
                onClick={() => signOut({ callbackUrl: "/" })}
                className="w-full py-4 bg-red-500 text-white font-bold rounded-xl shadow-lg shadow-red-100"
              >
                LOGOUT
              </button>
            ) : (
              <Link
                href="/login"
                onClick={() => setOpen(false)}
                className="block w-full py-4 bg-cyan-600 text-white text-center font-bold rounded-xl shadow-lg shadow-cyan-100"
              >
                LOGIN
              </Link>
            )}
